    ...
    if ($_SERVER["REQUEST_METHOD"] == "POST") {
        if (isset($_POST['action'])) {
            ...
            if ($action === 'update') {
                ...
                $status = $_POST['status']; // Retrieve selected option from dropdown
        ...
        <?php
            ...
            if ($result->num_rows > 0) {
                while($row = $result->fetch_assoc()) {
                    ...
                    echo "<option value='Negative' " . ($row["FeedbackStatus"] == 'Negative' ? 'selected' : '') . ">Negative</option>";
                    echo "<option value='Positive' " . ($row["FeedbackStatus"] == 'Positive' ? 'selected' : '') . ">Positive</option>";
                    echo "<option value='Neutral' " . ($row["FeedbackStatus"] == 'Neutral' ? 'selected' : '') . ">Neutral</option>";
                    echo "</select>";
                    echo "</td>";
                    echo "<td>";
